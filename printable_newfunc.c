...
/* Now that we've defined an interface, we can implement more complex behavior on the interface
type rather than the concrete types. This is a 2-level hierarchy -- happily, it never goes above 2 levels,
as an interface can't implement another interface. */
...
/* Now, in main, we'll run printable_print and printable_verbose_print on an int and a double: */
